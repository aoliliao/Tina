...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        # Define the CLIP encoder for conditioning
        self.clip_model, self.clip_image_preprocess = clip.load("ViT-B/32")
    ...
    def configure_optimizers(nn_module, lr, wd, betas):
        ...
        decay = set(param_dict.keys() - no_decay)
    ...
    @staticmethod
    def build_splits(parameter_sizes, split_policy='layer_by_layer', chunk_size=None):
        """
...
class Gpt(nn.Module):
    ...
    """
    The G.pt model for text-to-model personalization.
    """
    ...
    def compute_token_sizes(self, parameter_sizes, parameter_names, num_frequencies):
        ...
        # Account for the text tokenization of classification
        input_parameter_sizes.extend([[512] for _ in range(5)])
        input_parameter_names.extend([f'class_{c}_embedding' for c in range(5)])

        # input_parameter_sizes.extend([[512] for _ in range(20)])
        # input_parameter_names.extend([f'class_{c}_embedding' for c in range(20)])

        # Account for the scalar inputs (diffusion timestep):
        ...
        print('input_parameter_sizes', input_parameter_sizes)
    ...
    def forward(self, x, t, classes):
        ...
        classes: (10, 1), text prompts of the targeted personalized classes
        ----------------------------------------------
        ...
        # print(classes)
        with torch.no_grad():
            text_embeddings = [self.decoder.clip_model.encode_text(clip.tokenize(class_text).cuda()) for class_text in classes]

        # print(text_embeddings[0].shape)
        # print(classes)

        inp = [x]
        inp.extend(text_embeddings)
        inp.extend([t_embedding]) 
