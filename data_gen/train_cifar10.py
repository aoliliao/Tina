...
class ConvNet_5(nn.Module):
    def __init__(self, ws=[16, 32]):
        ...
        self.conv1 = nn.Conv2d(1, ws[0], kernel_size=3, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(ws[0], ws[1], kernel_size=3, stride=1, padding=1, bias=False)

        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(ws[1], 5)
...
class ConvNet_20(nn.Module):

    def __init__(self, ws=[16, 32]):
        super(ConvNet_20, self).__init__()
        self.conv1 = nn.Conv2d(3, ws[0], kernel_size=3, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(ws[0], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(ws[1], 20)
...
class ConvNet10k(nn.Module):
    def __init__(self, ws=[16, 32]):
        super(ConvNet10k, self).__init__()
        self.conv1 = nn.Conv2d(3, ws[0], kernel_size=3, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(ws[0], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        #
        self.conv3 = nn.Conv2d(ws[1], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        #
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(ws[1], 10)
...
class ConvNet33k(nn.Module):

    def __init__(self, ws=[16, 32, 64]):
        super(ConvNet33k, self).__init__()
        self.conv1 = nn.Conv2d(3, ws[0], kernel_size=3, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(ws[0], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        self.conv3 = nn.Conv2d(ws[1], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        self.conv4 = nn.Conv2d(ws[1], ws[2], kernel_size=3, stride=1, padding=1, bias=False)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(ws[2], 10)
...
class ConvNet76k(nn.Module):

    def __init__(self, ws=[16, 32, 64]):
        super(ConvNet76k, self).__init__()
        self.conv1 = nn.Conv2d(3, ws[2], kernel_size=3, stride=2, padding=1, bias=False)
        # self.conv2 = nn.Conv2d(ws[0], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        # self.conv3 = nn.Conv2d(ws[1], ws[1], kernel_size=3, stride=1, padding=1, bias=False)
        # self.conv4 = nn.Conv2d(ws[1], ws[2], kernel_size=3, stride=1, padding=1, bias=False)
        self.conv5 = nn.Conv2d(ws[2], ws[2], kernel_size=3, stride=1, padding=1, bias=False)
        self.conv6 = nn.Conv2d(ws[2], ws[2], kernel_size=3, stride=1, padding=1, bias=False)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(ws[2], 10)
...
# Data path
_DATA_PATH = "/home/lzx/Dataset/cifar"
...
class Cifar10(torch.utils.data.Dataset):
    ...
    def __init__(self, split):
        ...
        torchvision.datasets.CIFAR10(root=_DATA_PATH, download=False)
...
def save_checkpoint_lmdb(
    ...
    # 这里是一个run的json函数 用模型的dir作为key 来存储模型的Meta data
